...
def decide_delta(scale, step_min=1.0, step_max=8, eps=1e-6):
    s_min = scale.amin(dim=[2,3], keepdim=True)
    s_max = scale.amax(dim=[2,3], keepdim=True)
    step = step_max - (scale - s_min) * (step_max - step_min) / (s_max - s_min + eps)
...
class ICM(CompressionModel):
    ...
    def compress(self, x):
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            if mu is not None:
                outputs -= mu
            outputs = torch.round(outputs / delta)  # apply custom quant step
            y_q_slice = outputs.int()
            y_hat_slice = y_q_slice * delta + mu
    ...
    def decompress(self, strings, shape):
        ...
        for slice_index in range(self.num_slices):
            ...
            if mu is not None:
                y_hat_slice = rv.type_as(mu) * delta + mu
            else:
                y_hat_slice = rv.type(torch.float) * delta
